  ...
  return (
    <div className="w-full">
      <div className="relative max-w-screen-xl mx-auto text-center">
        <h1 className="text-[30vw] leading-none text-center tracking-tight mt-8 select-none">WORK</h1>
        <div className="absolute top-0 w-full h-full">
            {images.map((elem,index)=>elem.isActive &&  <div key={index}> <img key={index} className="absolute w-60 rounded-l -translate-x-[50%] -translate-y-[50%]" style={{top:elem.top, left:elem.left}} src={elem.url} alt="" /></div>)}
        </div>
      </div>
    </div>
  );
